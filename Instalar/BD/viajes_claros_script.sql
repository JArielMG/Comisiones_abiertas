...
CREATE TABLE `comisiones_detalle` (
  ...
  `tabla` varchar(50) DEFAULT NULL,
  `campo` varchar(50) DEFAULT NULL,
  `valor_texto` varchar(2000) DEFAULT NULL,
  `valor_numerico` double DEFAULT NULL,
  `valor_fecha` datetime DEFAULT NULL,
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `get_aprobador_usuario` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8 */ ;
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`viajes_admin`@`localhost` PROCEDURE `get_aprobador_usuario`(IN inUsuario VARCHAR(254), IN idFlujo INT)
BEGIN

DECLARE posicion VARCHAR(50);
DECLARE id_area INTEGER;

SET posicion =  (SELECT pos.nombre_posicion
				FROM viajes_claros.usuarios u
				INNER JOIN viajes_claros.personas p on u.id_persona = p.id_persona
				INNER JOIN viajes_claros.posiciones pos on p.id_posicion = pos.id_posicion
...
IF posicion LIKE 'HB%' AND idFlujo = 1 THEN
	SELECT -- CONCAT(p.titulo,' ',p.nombres,' ',p.apellido_paterno,' ',p.apellido_materno) AS aprobador, 
        DISTINCT a.nombre_area AS areaAprobacion
	FROM viajes_claros.configuracion_aprobacion ca
	INNER JOIN viajes_claros.jerarquias j on ca.id_jerarquia = j.id_jerarquia
	INNER JOIN viajes_claros.jerarquia_miembros jm on j.id_jerarquia = jm.id_jerarquia
	INNER JOIN viajes_claros.usuarios u on jm.id_usuario = u.id_usuario
	INNER JOIN viajes_claros.areas a on u.id_area = a.id_area
	INNER JOIN viajes_claros.personas p on u.id_persona = p.id_persona
	WHERE id_flujo = idFlujo AND j.nombre_jerarquia = 'DGA';
ELSEIF posicion NOT LIKE 'HB%' AND idFlujo = 1 THEN
	SELECT -- CONCAT(p.titulo,' ',p.nombres,' ',p.apellido_paterno,' ',p.apellido_materno) AS aprobador, 
       DISTINCT a.nombre_area AS areaAprobacion
	FROM viajes_claros.configuracion_aprobacion ca
	INNER JOIN viajes_claros.jerarquias j on ca.id_jerarquia = j.id_jerarquia
	...
	INNER JOIN viajes_claros.areas a on u.id_area = a.id_area
	INNER JOIN viajes_claros.personas p on u.id_persona = p.id_persona
	WHERE id_flujo = idFlujo AND ca.id_area = id_area;
ELSE
	SELECT -- CONCAT(p.titulo,' ',p.nombres,' ',p.apellido_paterno,' ',p.apellido_materno) AS aprobador, 
        DISTINCT a.nombre_area AS areaAprobacion
	FROM viajes_claros.configuracion_aprobacion ca
...
SET @query = CONCAT('SELECT ', @IdColumn, ' , ', campo, ' FROM ', tabla, ' ORDER BY ', campo);
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `get_comisiones_por_id_usuario` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8 */ ;
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`viajes_admin`@`localhost` PROCEDURE `get_comisiones_por_id_usuario`(IN idPersona INT(11))
BEGIN
SELECT c.id_comision AS idComision,
c.estatus AS estatus,
DATE_FORMAT(fechaViaje.valor_fecha,'%d/%m/%Y') AS fechaSalida,
DATE_FORMAT(fechaRegreso.valor_fecha,'%d/%m/%Y') AS fechaRegreso,
pais.valor_texto AS paisDestino,
ciudad.valor_texto AS ciudadDestino
FROM  comisiones c
INNER JOIN comisiones_detalle fechaViaje ON c.id_comision = fechaViaje.id_comision AND fechaViaje.campo = 'fecha_salida_comision'
INNER JOIN comisiones_detalle fechaRegreso ON c.id_comision = fechaRegreso.id_comision AND fechaRegreso.campo = 'fecha_regreso_comision'
INNER JOIN comisiones_detalle pais ON c.id_comision = pais.id_comision AND pais.campo = 'pais_destino'
INNER JOIN comisiones_detalle ciudad ON c.id_comision = ciudad.id_comision AND ciudad.campo = 'ciudad_destino'
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `get_comision_por_id_comision` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8 */ ;
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`viajes_admin`@`localhost` PROCEDURE `get_comision_por_id_comision`(idComision INT(11))
BEGIN
SELECT c.id_comision, c.estatus, c.id_dependencia, c.id_persona, c.id_usuario
FROM comisiones c
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `get_existe_comision_detalle` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8 */ ;
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`viajes_admin`@`localhost` PROCEDURE `get_existe_comision_detalle`(IN idComision INT(11),IN tabla varchar(50),IN campo varchar(50))
BEGIN	
SELECT cd.campo
FROM comisiones_detalle cd
WHERE cd.id_comision=idComision AND cd.tabla = tabla AND cd.campo = campo;
...
MAX(CASE when ab.id_flujo = 1 then f.descripcion END) 'f1',
MAX(CASE when ab.id_flujo = 2 then f.descripcion END) 'f2',
MAX(CASE when ab.id_flujo = 3 then f.descripcion END) 'f3',
MAX(CASE when ab.id_flujo = 4 then f.descripcion END) 'f4'
FROM  comisiones c
...
WHERE p.id_tipo_persona<>3 AND d.predeterminada = 1
...
IF (SELECT id_comision FROM viajes_claros_instancias vi
	INNER JOIN dependencias d ON vi.id_dependencia = d.id_dependencia
    WHERE id_viaje=idViaje and d.predeterminada = 1) is not null THEN
	SELECT p.id_persona, p.nombres, p.apellido_paterno, p.apellido_materno, p.titulo,
	p.email, p.id_categoria, cat.nombre_categoria, p.id_tipo_persona, t.descripcion,
	u.id_area, a.nombre_area, u.id_dependencia
	FROM personas p
...
BEGIN
	...
	FROM valores_dinamicos WHERE id_lista=idLista and activo = 1
	ORDER BY valor;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
SELECT IFNULL(b.despliegue, d.despliegue) AS despliegue,
	CASE WHEN v.valor_texto IS NULL THEN (
		...
		ELSE (CASE WHEN LOWER(v.campo) LIKE '%gasto%' OR LOWER(v.campo) LIKE '%costo%' OR 
			LOWER(v.campo) LIKE '%importe%' OR LOWER(v.campo) LIKE 'viaticos_%' OR 
            LOWER(v.campo) LIKE '%monto%' THEN 
					CONCAT('$', FORMAT(v.valor_numerico, 2), ' MXN')
             ELSE (CASE WHEN LOWER(v.campo) LIKE '%tarifa%' THEN
					CONCAT('$', FORMAT(v.valor_numerico, 2), ' ',(SELECT vd.valor AS valor
                        FROM valores_dinamicos vd 
						INNER JOIN campos_dinamicos cd ON vd.id_lista = cd.id_lista
						INNER JOIN viajes_claros_detalle vcd ON cd.nombre_campo = vcd.campo
						WHERE vcd.id_viaje = v.id_viaje and LOWER(vcd.campo) like '%moneda%' and vd.codigo = vcd.valor_texto))
			  ...
			  ELSE v.valor_numerico END) END) END) END) 
	ELSE (CASE WHEN EXISTS (
						SELECT vd.valor AS valor
                        FROM valores_dinamicos vd 
						INNER JOIN campos_dinamicos cd ON vd.id_lista = cd.id_lista
					...
					(
						SELECT 
							...
							LOWER(v.campo) LIKE '%importe%' OR  LOWER(v.campo) LIKE 'viaticos_%' OR 
							LOWER(v.campo) LIKE '%monto%' THEN 
								CONCAT('$', FORMAT(CONVERT(vd.valor,SIGNED INTEGER), 2), ' MXN')
		...
		ELSE (CASE WHEN LOWER(v.campo) LIKE '%tarifa%' THEN
						CONCAT('$', FORMAT(v.valor_texto, 2), ' ',(SELECT vd.valor AS valor
                        FROM valores_dinamicos vd 
						INNER JOIN campos_dinamicos cd ON vd.id_lista = cd.id_lista
						INNER JOIN viajes_claros_detalle vcd ON cd.nombre_campo = vcd.campo
						WHERE vcd.id_viaje = v.id_viaje and LOWER(vcd.campo) like '%moneda%' and vd.codigo = vcd.valor_texto))
					ELSE v.valor_texto END) END) END as valor
	FROM viajes_claros_detalle v 
...
IN campo VARCHAR(50),IN valorTexto VARCHAR(2000), IN valorNumerico DOUBLE, IN valorFecha DATETIME)
BEGIN
IF ((valorTexto IS NOT NULL OR valorFecha IS NOT NULL) AND valorNumerico = 0) THEN
...
BEGIN
	select cd.id_detalle, cd.id_comision, cd.tabla, cd.campo
		  ,CASE WHEN EXISTS (
						SELECT vd.valor AS valor
                        FROM flujos_instancias fli
						INNER JOIN comisiones_detalle cod ON fli.id_comision = cod.id_comision
						INNER JOIN campos_dinamicos cad ON cod.campo = cad.nombre_campo
                        INNER JOIN valores_dinamicos vd  ON cad.id_lista = vd.id_lista
						WHERE fli.id_instancia = instancia AND cod.campo = cd.campo and vd.codigo = cd.valor_texto
                    ) THEN 
					(
						SELECT vd.valor AS valor
                        FROM flujos_instancias fli
		...
		ELSE cd.valor_texto END as valor_texto, cd.valor_numerico, cd.valor_fecha, cf.etiqueta, cf.subtipo
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`viajes_admin`@`localhost` PROCEDURE `update_comision_detalle`(IN idComision INT(11),IN tab varchar(50),IN camp varchar(50),IN valorTexto VARCHAR(2000), IN valorNumerico DOUBLE, IN valorFecha DATETIME)
BEGIN	
IF ((valorTexto IS NOT NULL OR valorFecha IS NOT NULL) AND valorNumerico = 0) THEN
