 ...
 */
myApp.controller('BuscadorController', ['$scope', '$rootScope', '$location','$log', 'BuscadorService', function ($scope, $rootScope, $location, $log, BuscadorService) {
	...
	if ($rootScope.anioConsulta!=null&&$rootScope.anioConsulta!='')
        ...
        BuscadorService.realizaBusquedaComplete($scope.dependencia).then(function (d) {
            ...
            for (var i = 0; i < d.length; i++) {
                ...
                for (var j = 0; j < d[i].datos.length; j++) {
                    if (d[i].datos[j].includes("\"")){
                    	 var text = d[i].datos[j].toString();
       ...
       /**
        ...
        $scope.saveJSON = function () {
            ...
            a.download = "comisiones_json.txt";
        ...
        $scope.saveXML = function () {
            ...
            if (window.navigator.msSaveOrOpenBlob) {
                console.log("IE!");
                window.navigator.msSaveOrOpenBlob(blob, "comisiones_xml.xml");
            } else {
                console.log("A true browser");
                a.download = "comisiones_xml.xml";
                a.href = window.URL.createObjectURL(blob);
